    ...
    function fetch_records_remote() {
        ...
        while (true) {
            ...
            foreach ($query["ResourceRecordSets"] as $route53_record) {
                ...
                if (isset($route53_record["AliasTarget"])) {
                    $tmp["name"] = rtrim($route53_record["Name"], '.');
//                        $tmp["type"] = $route53_record["Type"];
                    $tmp["type"] = 'ALIAS';
                    $tmp["content"] = $route53_record["AliasTarget"]['DNSName'];
                    $tmp["prio"] = '0';
                    $tmp["ttl"] = '120';
...
// end of fetch_records_remote()
    ...
    function action_sync_records() {
        ...
        foreach ($ids_route53_deleted as $id) {
            ...
            $data_change_batch[] = $this->_alias_format($tmp);
        }
...
//				die("debug");
    ...
    function _alias_format($tmp) {
        if (isset($tmp['ResourceRecordSet']['Type']) && $tmp['ResourceRecordSet']['Type'] == 'ALIAS') {
            $tmp['ResourceRecordSet']['Type'] = 'A';
            $tmp['ResourceRecordSet']['AliasTarget'] = array(
                'HostedZoneId' => str_replace('/hostedzone/','',$this->aws_zone_id),
                'DNSName' => 'dualstack.' . str_replace('dualstack.', '', $tmp['ResourceRecordSet']['ResourceRecords'][0]['Value']),
                'EvaluateTargetHealth' => false,
            );
            unset($tmp['ResourceRecordSet']['ResourceRecords']);
            unset($tmp['ResourceRecordSet']['TTL']);
        }
